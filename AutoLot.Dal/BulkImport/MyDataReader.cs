...
public sealed class MyDataReader<T>:IMyDataReader<T>
{
    ...
    public MyDataReader(List<T> records, SqlConnection connection, string schema, string tableName)
    {
        ...
        for(int i = 0; i < schemaTable.Rows.Count; i++)
        {
            ...
            //_nameDictionary[i]=colName;
            _nameDictionary.Add(i, colName);
        }
    }
    ...
    {
        T item = Records[_currentIndex];
        string colName = _nameDictionary[i];
        
        return _propertyInfos
        .First(x=>x.Name.Equals(colName,StringComparison.OrdinalIgnoreCase))
        .GetValue(item);
